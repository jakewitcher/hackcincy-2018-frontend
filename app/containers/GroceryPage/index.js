...
export class GroceryPage extends React.PureComponent {
  ...
  render() {
    console.log('this.props.currentPlace: ', this.props.currentPlace); //  eslint-disable-line no-console
    return (
      <Wrapper>
        <ImageContainer>
          <a href="/map">
            <SVG src="http://cdn.onlinewebfonts.com/svg/img_170454.png" />
            <Back>Back to Map</Back>
          </a>
          <Kroger src="https://s3.amazonaws.com/freebiesupply/large/2x/kroger-logo-png-transparent.png" />
          <Clicklist src="https://www.kroger.com/asset/retina-store-logo" />
        </ImageContainer>
        <Layout>
          {this.props.currentPlace &&
            this.props.currentPlace.groceries.map((el, i) => (
              <GroceryListItem
                key={i}
                name={el.name}
                price={el.price}
        ...
        <Panel>
          <h1>
            <span style={{ fontSize: '16px', color: '#585858' }}>TOTAL:</span>{' '}
            {new Intl.NumberFormat('en', {
              style: 'currency',
              currency: 'USD',
            }).format(this.state.allTotals)}
          </h1>
